...
class SimplicityWidgetItem extends WP_Widget {
  function SimplicityWidgetItem() {
      parent::__construct(false, $name = '[S] 新着・人気記事', array('description' => 'トップページで「人気理事「人気記事」リストを、それ以外のページで「新着記事」リストを表示するSimplicityウィジェットです。（※要Wordpress Popular Postsプラグイン）'));
    }
...
class SimplicityNewEntryWidgetItem extends WP_Widget {
     function SimplicityNewEntryWidgetItem() {
         parent::__construct(false, $name = '[S] 新着記事', array('description' => '新着記事リストを表示するSimplicityウィジェットです。'));//ウイジェット名
    }
    function widget($args, $instance) {
        extract( $args );
        //タイトル名を取得
...
class SimplicityPopularPostsCategoryWidgetItem extends WP_Widget {
  function SimplicityPopularPostsCategoryWidgetItem() {
      parent::__construct(false, $name = '[S] 人気記事', array('description' => '人気記事リストを表示するSimplicityウィジェットです。（※要Wordpress Popular Postsプラグイン）'));
    }
    function widget($args, $instance) {
        extract( $args );
...
class SimplicitySocialFollowWidgetItem extends WP_Widget {
  function SimplicitySocialFollowWidgetItem() {
      parent::__construct(false, $name = '[S] SNSフォローボタン', array('description' => 'SNSサービスのフォローアイコンボタンを表示するSimplicityウィジェットです。'));
    }
    function widget($args, $instance) {
      extract( $args );
...
class MobileTextWidgetItem extends WP_Widget {
   function MobileTextWidgetItem() {
     parent::__construct(false, $name = '[S] モバイル用テキストウィジェット', array('description' => 'モバイルのみで表示されるSimplicity用のテキストウィジェットです。'));//ウイジェット名
  }
  function widget($args, $instance) {
    extract( $args );
    //タイトル名を取得
...
class PcTextWidgetItem extends WP_Widget {
   function PcTextWidgetItem() {
     parent::__construct(false, $name = '[S] パソコン用テキストウィジェット', array('description' => 'パソコンのみで表示されるSimplicity用のテキストウィジェットです。'));//ウイジェット名
  }
  function widget($args, $instance) {
    extract( $args );
    //タイトル名を取得
...
class FacebookPageLikeWidgetItem extends WP_Widget {
   function FacebookPageLikeWidgetItem() {
     parent::__construct(false, $name = '[S] Facebookページ「いいね！」', array('description' => '投稿・個別ページのアイキャッチを利用したFacebookページへの「いいね！」ボタンを表示するSimplicity用ウィジェットです。（※「Facebookページ」のみで利用できます。「個人ページ」で設定されている場合は利用できません）'));//ウイジェット名
  }
  function widget($args, $instance) {
    extract( $args );
    //タイトル名を取得
...
class MobileAdWidgetItem extends WP_Widget {
  function MobileAdWidgetItem() {
    parent::__construct(false, $name = '[S] モバイル用広告ウィジェット', array('description' => 'モバイルのみで表示されるSimplicity用の広告ウィジェットです。（※アドセンスの場合は広告コードのみを記入してください。）'));//ウイジェット名
  }
  function widget($args, $instance) {
    extract( $args );
...
class PcAdWidgetItem extends WP_Widget {
  function PcAdWidgetItem() {
    parent::__construct(false, $name = '[S] パソコン用広告ウィジェット', array('description' => 'パソコンのみで表示されるSimplicity用の広告ウィジェットです。（※アドセンスの場合は広告コードのみを記入してください。）'));//ウイジェット名
  }
  function widget($args, $instance) {
    extract( $args );
    $ad = apply_filters( 'widget_ad_text', $instance['ad_text'] );
...
class PcDoubleAdsWidgetItem extends WP_Widget {
  function PcDoubleAdsWidgetItem() {
    parent::__construct(false, $name = '[S] パソコン用広告ダブルレクタングルウィジェット', array('description' => 'パソコンのみで表示されるSimplicity用のダブルレクタングル広告ウィジェットです。（※アドセンスの場合は広告コードのみを記入してください。）'));//ウイジェット名
  }
  function widget($args, $instance) {
    extract( $args );
